...
class notionFun(object):
    ...
    @staticmethod
    def add_page_heading_date_in_database_with_icon_new(dataTableId,icon_url,VP_Name,VP_ID,Comments,Path,s_date,e_date):
        '''
        '''
        url="https://api.notion.com/v1/pages"
        data=\
    ...
    @staticmethod
    def response_to_json_file(response,json_file_path=r"C:\Andy\pycode\Andy_Python\python\Notion\test.json"):
...
if os.path.exists(p_image_path):
    ...
    date=current_date()
    name= os.path.basename(p_image_path)
    file=p_image_path
    myDrive=MyGoogleDrive(service_account_file_path=service_account_file)
    imageid=myDrive.upload_or_update_file_return_id(name,file,folder_Id=folder_id)     
    imageUrl=f"https://drive.google.com/file/d/{imageid}/preview"

    if find_page_id==None:
        p_e_date=""
        notionFun.add_page_heading_date_image_in_database_with_icon_new(pageid_Navis_VP_Table,LOGO_URL,p_vp_name,p_vp_id,p_comments,p_path,p_s_date,p_e_date,date,imageUrl)
    else:
        if "close" in  p_vp_name.lower():
            ...
            if "close" not in str(currentTitle).lower() :
                ...
                notionFun.add_heading_date_image_in_page_after_blockid(find_page_id,blockid,date,imageUrl)
